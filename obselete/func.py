...
def avgNbEntities():
	...
	each = db.aggregateArticles([match, proj, group, sort])
	avg = 0.0; total = 0.0
	for e in each:
		avg += e['count']*e['_id']
		total += e['count']
...
def popularKeywords():
	...
	ret = db.aggregateArticles([match, unwind, group, sort, limit])
	for r in ret:
		print r
...
def contentRegexSource(st):
	...
	ret = db.aggregateArticles([match, group, sort])
	for r in ret:
		print r

	recentArticles = db.getLatestArticles(match['$match'], {'_id': 1}, limit=10)
	print [a['_id'] for a in recentArticles]
